...
- A **Help Center** page inside the panel, at `{panel}/help` and `{panel}/help/{slug}`, behind the panel's own auth and guard. It is a Filament page built from schema and layout components rather than a Blade layout, so it follows the panel's light and dark mode and the core stylesheet reaches the article body only. The left rail carries a search box and two tabs: **Contents**, the full tree the viewer may read, with collapsible sections that remember their own open state and the current article highlighted, and **Search**, the core searcher on a debounced query, up to 50 hits with highlighted snippets and their section path, with the core's rate-limit and no-results lines. The middle column shows breadcrumbs, the title and the body through the core renderer, with the other-language fallback notice, related articles, images and downloads as the drawer renders them, a not-found state for a missing or hidden slug and a pick-a-topic state at the bare `{panel}/help`. **On this page** lists the article's headings on the right and goes away when it has none
- `FinCodexPlugin::make()->helpCenterPlacement()` says where each panel offers its Help Center: `HelpCenterPlacement::UserMenu` (the default), `Navigation`, `Both` or `None`. The default is a labelled book entry in the user menu, directly after **Profile**; `Navigation` registers a sidebar item placed by four options of its own — `helpCenterNavigationGroup()` (no group), `helpCenterNavigationSort()` (`1000`), `helpCenterNavigationLabel()` (the translated **Help center**) and `helpCenterNavigationIcon()` (the book) — rather than by the `navigationGroup()` and `navigationSort()` the three authoring screens share, so the reading surface does not land in the **Help** group beside them. `None` withholds the two entries and nothing else: the page stays registered and `{panel}/help` keeps answering, so the drawer footer, the field hints, the global search and a bookmark all still reach it. Both entries go for a viewer the page's own gate refuses
- `ArticlePolicy::viewAllPanels()`, an ability unlike the other eight: it guards no screen, it widens what a viewer may read across panels, and it has no fallback to `viewAny`. It answers false by default; on a host with Filament Shield it answers the permission Shield generated for it, asked of Shield by name, and a host override at `App\Policies\ArticlePolicy` still replaces the method outright. A viewer it allows also gets a **Panel** select at the top of the Help Center's rail — this panel on arrival, each other panel, the articles that file under no panel, or **All panels** — and the tree and the hits re-render as a reader standing in that panel would see them. Every other viewer sees no select
- `FinCodexPlugin::make()->helpCenterPage()` swaps the Help Center for a host subclass, per panel, the way the article resource and the two settings pages are already swapped; a class that does not extend the shipped page is ignored
...
- A twelfth starter article, **The help center** (`help/help-center`), in English, German and Hungarian and attached to the page itself, so it is what the drawer's **This page** tab answers there and a fresh install no longer opens the Help Center on a coverage report flagging its own screen. It is written for the reader first — reaching the page, Contents, Search, breadcrumbs, **On this page**, the panel filter — and then for the administrator. **Getting help** links through to it, the first article-to-article link in the shipped set

### Changed

- **The core's public help center is off on a fin-codex install.** `fin-codex:install` writes `null` into the published `lin-codex.routes.help_center`, on a fresh host and on an upgrading one, so `/help` and `/help/{slug}` answer 404 and an external link or a bookmark to them stops working. Help now lives inside the panel, behind its login. A host who set their own prefix is shown it and asked before it is overwritten, and answering no leaves the public page exactly as it was; `fin-codex:uninstall` offers the prefix back. A host installed before 0.5.0 has one more thing to check — a published copy of the two panel views still resolves the core route by name and throws once the prefix is null — see the README's Upgrading section
- Requires `finity-labs/lin-codex` ^0.4.2, up from ^0.4. The core's 0.4.1 fixed the other half of the string user keys (`HasUuids`, `HasUlids`) that fin-codex 0.4.1 fixed on the panel side, and 0.4.2 added the test proving it, so a fresh 0.5.0 resolving the core's 0.4.0 would take the panel half of one fix without the core half. The 0.4 line is also what lets a null `routes.help_center` switch the public page off while the media, the API, the stylesheet, the drawer and the button keep working, and what reports the throwable behind a failed translation job to the host's error tooling
- **Help is scoped to the panel it is read in.** Inside a panel request a viewer sees general articles — those whose contexts name no panel — plus articles with a context in the current panel; another panel's articles, and the sections left holding nothing else, are hidden in the Help Center, the drawer's browse tab and search, the field hints and the panel's global search. A context's panel is its explicit `panel:` prefix, which is what a `HasHelp` declaration and the editor's panel select both write, so a context that names no panel restricts nobody. The rule is installed as lin-codex's `auth.gate` hook when the panel boots, chaining a hook the host already set, and outside a panel request — the JSON API, the media route, the coverage report — the core reads exactly as it did on 0.4. An article that used to show in two panels and now shows in one has a context naming the other; `viewAllPanels` is how a viewer is let past the rule
- Every help link fin-codex renders inside a panel resolves to that panel's own Help Center instead of the public page: the drawer footer's **Open help center**, the `CodexHelp` field-hint fallback href, the global search rows and the article-to-article links the core renderer writes into a body. The prefix is derived from the page class the panel actually registered and set for the length of the request, so `/admin/help/...` and `/staff/help/...` come out of the same code with no setter and no core change, and the SPA exception pattern follows it so the hint's drawer intercept keeps winning. Where there is no page to reach, the link goes rather than pointing nowhere: the drawer footer drops its whole action group when no URL could be built, on a simple-layout (guest) page, or for a viewer the page's gate refuses — and that viewer's global search row falls back to `?codex={slug}` on the page they are already on, which opens the drawer where they stand

### Fixed

- The editor's **Page class** picker offers the panel's own sign-in, registration, password-reset request, password-reset and email-verification pages, its profile page, and its two-factor setup page on a panel that has providers and requires them — under **Any panel** only, which is where they belong, since naming one panel would hide the article on every other. They are labelled from fin-codex's own English, German and Hungarian strings rather than shown as a raw class name, and badged as a third kind beside resources and pages. fin-codex's own starter articles have always bound to those screens; until now its editor could not reproduce them
- Changing a context row's **Panel** keeps a key the new panel still offers and clears only one it does not, with a short notice when it does. Switching to **Any panel** never clears, and a url row's pattern is never cleared. A resource registered in both panels no longer has to be picked again every time the panel beside it moves
- A row set to **Any panel** whose key belongs to exactly one panel, and a url row whose pattern sits under another panel's path, both show an inline warning naming that panel. The warnings appear as soon as the mismatch does, clear themselves when the row is fixed and never block a save; a shared class, a panel-less route, an auth page and an unclaimed path stay silent
- The article list's **Missing: {language}** filter counts a translation row with a blank title or body as missing, so it lists the same articles as the languages badge beside it and as both **Translate missing** actions. It used to ask only whether a row existed at all, which made a half-written translation missing to the badge and translated to the filter
- A completion notification whose **Open article** URL cannot be built — a panel with tenancy, whose routes all sit under a tenant segment the worker cannot name; a resource override the worker cannot route; a panel changed since the press — now arrives without its button instead of being lost entirely. Until now nothing was stored in that case and the only trace of the run was `fin-codex: could not store the translation notification` at `error`; the notification is now delivered, naming the article and the languages it gained, and the panel's log carries a `debug` line with the article id, the panel id and the throwable
