...
trait CanRegisterPlugin
{
    protected function registerPlugin(string $panelPath): void
    {
        ...
        if (str_contains($content, $pluginCall)) {
            $this->components->warn('FinMailPlugin is already registered in this panel provider.');
        ...
        $this->components->info('FinMailPlugin has been registered successfully!');
    }
    ...
    protected function createPluginsBlock(string $content, string $pluginCall): string
    {
        ...
        if ($pos === false) {
            $this->components->warn('Could not find a suitable insertion point for ->plugins(). Please register FinMailPlugin::make() manually.');
